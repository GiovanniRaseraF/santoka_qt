...
void hp_faultwarning_loader::loadfromfile(QString filename)
{
    ...
    // get packets
    QJsonValue value1 = sett2.value(QString("packets"));
    QJsonArray elempackets = value1.toArray();

    // every packet has channel and bits
    for(auto pack : elempackets){
        // channel estraction
        QString channel =  pack.toObject().value(QString("_canchannel")).toString();
        ...
        QJsonValue elems = pack.toObject().value(QString("elem"));
